 ...
 */
GBasic::GBasic(short steps, short dir_pin, short step_pin, Shiftduino& param, int index)
    : GBasic(steps, dir_pin, step_pin, PIN_UNCONNECTED, param, index) {
}

GBasic::GBasic(short steps, short dir_pin, short step_pin, short enable_pin, Shiftduino& param, int index)
    : motor_steps(steps), dir_pin(dir_pin), step_pin(step_pin), enable_pin(enable_pin), _sd(param), _index(index) {
    steps_to_cruise = 0;
    steps_remaining = 0;
    dir_state = 0;
    steps_to_brake = 0;
...
GBasic::GBasic(short steps,
               ...
               Shiftduino& param,
               int index)
    : GBasic(steps, dir_pin, step_pin, PIN_UNCONNECTED, mode0_pin, mode1_pin, mode2_pin, param, index) {
}
...
void GBasic::begin(float rpm, short microsteps) {
    ...
    if (!IS_CONNECTED(mode0_pin) || !IS_CONNECTED(mode1_pin) || !IS_CONNECTED(mode2_pin)) {
        return;
    }
...
long GBasic::getTimeForMove(long steps) {
    ...
    switch (profile.mode) {
        case LINEAR_SPEED:
            ...
            t = (cruise_steps / (microsteps * speed)) + sqrt(2.0 * steps_to_cruise / profile.accel / microsteps) +
 ...
 */
void GBasic::startRotate(long deg) {
 ...
 * due to inclusion of float support.
 */
void GBasic::startRotate(double deg) {
    startMove(calcStepsForRotation(deg));
}
 ...
 */
void GBasic::setEnableActiveState(short state) {
...
short GBasic::getMaxMicrostep() {
    return GBasic::MAX_MICROSTEP;
}
...
size_t GBasic::getMicrostepTableSize() {
    return sizeof(GBasic::MS_TABLE);
}
