...
StepperDriver::StepperDriver(int number_of_steps, int step_division, int en_pin,
                             int cw_pin, int clk_pin, Shiftduino &param,
                             int index)
    : _sd(param), _index(index) {
...
void StepperDriver::powerEnable(bool ena) {
  _sd.setPin(((_index * 8) + en_pin), ena ? LOW : HIGH);
  /*
  if (ena) {
...
void StepperDriver::step(long steps_to_move, long steps_acc, long steps_dec) {
  ...
  for (long i = (abs(steps_to_move) - abs(steps_acc) - abs(steps_dec)); i > 0;
       i--) {
    move();
  }
...
void StepperDriver::setDirection(long steps_to_move) {
  _sd.setPin(((_index * 8) + cw_pin), (steps_to_move < 0) ? HIGH : LOW);
  /*
  if (steps_to_move < 0) {
...
void StepperDriver::move() {
  _sd.setPin(((_index * 8) + clk_pin), HIGH);
...
void StepperDriver::moveInterval(unsigned long target_delay) {
  ...
  _sd.setPin(((_index * 8) + clk_pin), LOW);
