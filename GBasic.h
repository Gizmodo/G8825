 ...
 */
#define STEP_PULSE(steps, microsteps, rpm)                                     \
  (60.0 * 1000000L / steps / microsteps / rpm)
...
public:
  ...
  static inline void delayMicros(unsigned long delay_us,
...
protected:
  short mode0_pin = PIN_UNCONNECTED;
  short mode1_pin = PIN_UNCONNECTED;
  short mode2_pin = PIN_UNCONNECTED;
  // Get the microstep table
  const uint8_t *getMicrostepTable();
  size_t getMicrostepTableSize();
  /*
   * Motor Configuration
   */
  ...
  long cruise_step_pulse; // step pulse duration for constant speed section (max
                          // rpm)
  ...
  // this is internal because one can call the start methods while CRUISING to
  // get here
...
public:
   ...
   */
  GBasic(short steps, short dir_pin, short step_pin, Shiftduino &param,
         int index);
  GBasic(short steps, short dir_pin, short step_pin, short enable_pin,
         Shiftduino &param, int index);
  GBasic(short steps, short dir_pin, short step_pin, short mode0_pin,
         short mode1_pin, short mode2_pin, Shiftduino &param, int index);
  GBasic(short steps, short dir_pin, short step_pin, short enable_pin,
         short mode0_pin, short mode1_pin, short mode2_pin, Shiftduino &param,
         int index);
  /*
   * Initialize pins, calculate timings etc
   */
  void beginChild(float rpm = 60, short microsteps = 1);
  ...
  short setMicrostepChild(short microsteps);
  short getMicrostep(void) { return microsteps; }
  short getSteps(void) { return motor_steps; }
  /*
  ...
  short getAcceleration(void) { return profile.accel; }
  short getDeceleration(void) { return profile.decel; }
  /*
  ...
  inline void rotate(int deg) { rotate((long)deg); };
  /*
  ...
  /*
   * Turn off/on motor to allow the motor to be moved by hand/hold the position
   * in place
   */
   ...
   * The flow has two parts - start/initiate followed by looping with
   * nextAction. See NonBlocking example.
   */
  /*
   * Initiate a move over known distance (calculate and save the parameters)
   * Pick just one based on move type and distance type.
   * If time (microseconds) is given, the driver will attempt to execute the
   * move in exactly that time by altering rpm for this move only (up to preset
   * rpm).
   */
  ...
  /*
   * Toggle step at the right time and return time until next change is needed
   * (micros)
   */
   ...
   */
  long getStepsCompleted(void) { return step_count; }
  /*
   * Get the number of steps remaining to complete the move
   * This is always a positive number
   */
  long getStepsRemaining(void) { return steps_remaining; }
  /*
   * Get movement direction: forward +1, back -1
   */
  int getDirection(void) { return (dir_state == HIGH) ? 1 : -1; }
  /*
